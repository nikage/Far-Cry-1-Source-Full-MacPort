...
void CXClient::OnXContextSetup(CStream &stm)
{
	...
	if (!bFastPathSpListen)
	{
		...
		m_pLog->Log("\003[GameCheckpoint] Client_OnXContextSetup ixsystem_LoadLevel_ok folder='%s' mission='%s'",
			m_GameContext.strMapFolder.c_str(), m_GameContext.strMission.c_str());
		...
		if (!m_pGame->m_bEditor && !m_pGame->IsServer())
			m_pLog->Log("\003[GameCheckpoint] Client_OnXContextSetup checksum_verified folder='%s' ck=%u",
...
void CXClient::OnMapChanged()   
{
	...
	m_pLog->Log("\003[GameCheckpoint] Client_OnMapChanged pending_local_player level='%s' mp=%d",
...
void CXClient::OnMapChangedReally()   
{
  m_bMapConnecting = false;
	...
	const char *lvl = m_pGame && m_pGame->g_LevelName ? m_pGame->g_LevelName->GetString() : "";
	m_pLog->Log("\003[GameCheckpoint] Client_OnMapChangedReally playable_hooks level='%s' mp=%d checkpoint_load=%d",
