...
bool CXSystemServer::LoadLevel(const char *szLevelDir,const char *szMissionName, bool bEditor)
{
	...
	m_pLog->Log("\003[GameCheckpoint] IXSystem_Server_LoadLevel enter dir='%s' mission='%s' editor=%d",
	...
	m_pLog->Log("\003[GameCheckpoint] server_before_GameRules_OnAfterLoad dir='%s'", szLevelDir);
	m_pXServer->GetRules()->OnAfterLoad();
	m_pLog->Log("\003[GameCheckpoint] server_after_GameRules_OnAfterLoad dir='%s'", szLevelDir);
	...
	m_pLog->Log("\003[GameCheckpoint] server_after_OnAfterServerLoadLevel dir='%s' port=%d",
	...
	m_pLog->Log("\003[GameCheckpoint] IXSystem_Server_LoadLevel exit ok dir='%s' mission='%s'",
		szLevelDir, ms);
	return true;
}
